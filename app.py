...
class LanguageLearningApp:
    ...
    def _load_detector_model(self) -> None:
        ...
        try:
            self.detector = pipeline(
                model=Config.DETECTOR_CHECKPOINT, task="zero-shot-object-detection"
            )
            self.logger.info(
                f"Successfully loaded detector model: {Config.DETECTOR_CHECKPOINT}"
            )
        except Exception as e:
    ...
    def _register_routes(self) -> None:
        ...
        self.app.add_url_rule(
            "/<string:language>/image", "language_image_page", self.language_image_page
        )

        # API routes
        self.app.add_url_rule(
            "/api/text-chat", "text_chat", self.text_chat, methods=["POST"]
        )
        self.app.add_url_rule(
            "/api/voice-chat", "voice_chat", self.voice_chat, methods=["POST"]
        )
        self.app.add_url_rule("/api/audio/<audio_id>", "play_audio", self.play_audio)
        self.app.add_url_rule(
            "/api/image_guess", "image_guess", self.image_guess, methods=["POST"]
        )
    ...
    def language_image_page(self, language: str):
        ...
        try:
            ...
            return render_template(
                f"{language}_image.html", img_file=img_url, language=language
            )
    ...
    def _fetch_random_image(self) -> Optional[str]:
        ...
        try:
            response = requests.get(
                "https://picsum.photos/400/400", timeout=10, allow_redirects=True
            )
    ...
    def _get_or_create_cached_audio(
        self, cache_key: str, text: str, language: str
    ) -> str:
        """
    ...
    def _process_voice_conversation(self, audio_file, language: str) -> Dict[str, Any]:
        ...
        try:
            ...
            transcribed_text = self.transcription_model.transcribe_audio(
                audio_file=input_audio_path, language=language
            )
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to clean up audio file {input_audio_path}: {e}"
                )

    def _process_image_guess(
        self, audio_file, language: str, image_url: str
    ) -> Dict[str, Any]:
        """
    ...
    def _detect_objects_in_image(
        self, image: Image.Image, candidate_labels: list
    ) -> Image.Image:
        """
    ...
    def run(
        self, debug: bool = True, port: int = 5170, host: str = "127.0.0.1"
    ) -> None:
        """
